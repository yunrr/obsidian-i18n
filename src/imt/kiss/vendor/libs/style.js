// [obsidian-i18n] 移植补丁：移除 @emotion/css 依赖，改为纯 CSS 文本生成
import {
  OPT_STYLE_NONE,
  OPT_STYLE_LINE,
  OPT_STYLE_DOTLINE,
...
// 动画名由 genTextClass 输出的 @keyframes 提供
const gradientFlow = "kiss-gradient-flow";

const blink = "kiss-blink";

const glow = "kiss-glow";
...
export const genTextClass = (customStyles = []) => {
  ...
  // [obsidian-i18n] 纯 CSS 实现：返回固定类名映射与完整样式表字符串，
  // 由调用方负责注入 <style>。嵌套 &:hover 依赖 Chromium 原生 CSS Nesting。
  const textClass = {};
  let textStyles = `
...
`;
  Object.entries(styles).forEach(([k, v]) => {
    const className = `kiss-style-${k}`;
    textClass[k] = className;
    textStyles += `
.${className} {
  ${v}
}
