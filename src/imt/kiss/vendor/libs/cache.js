...
import { isExt, isObsidian } from "./client";
import { isBg } from "./browser";
import {
  obsidianCacheGet,
  obsidianCachePut,
  obsidianCacheClear,
} from "../../adapters/obsidian-cache";
...
export const tryClearCaches = async () => {
  try {
    // [obsidian-i18n] Obsidian 下使用内存缓存适配器
    if (isObsidian) {
      obsidianCacheClear();
      return;
    }
...
export const getHttpCache = async ({ input, init, expect }) => {
  // [obsidian-i18n] Obsidian 下 caches API 不可靠，改用内存缓存适配器
  if (isObsidian) {
    return obsidianCacheGet(input, init, expect);
  }
  try {
...
export const putHttpCache = async ({
  ...
  // [obsidian-i18n] Obsidian 下 caches API 不可靠，改用内存缓存适配器
  if (isObsidian) {
    return obsidianCachePut(input, init, data, maxAge);
  }
  try {
