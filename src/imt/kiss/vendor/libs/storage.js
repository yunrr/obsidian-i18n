...
import { isExt, isGm, isObsidian } from "./client";
import { browser } from "./browser";
import {
  obsidianStorageGet,
  obsidianStorageSet,
  obsidianStorageDel,
} from "../../adapters/obsidian-storage";
...
async function set(key, val) {
  // [obsidian-i18n] Obsidian 环境：插件 data.json 承载全部 kiss 存储键
  if (isObsidian) {
    return obsidianStorageSet(key, val);
  }
  if (isExt) {
...
const mergeSettingWithDefault = (setting) => {
  // [obsidian-i18n] 过滤显式 undefined 字段：迁移函数会对缺失字段返回显式 undefined 键，
  // 直接展开会把 DEFAULT_SETTING 里的 transApis 等默认值覆盖成 undefined。
  const cleanSetting = Object.fromEntries(
    Object.entries(setting || {}).filter(([, value]) => value !== undefined)
  );
  const mergedSetting = {
    ...DEFAULT_SETTING,
    ...cleanSetting,
